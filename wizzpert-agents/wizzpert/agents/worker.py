...
from wizzpert.protocol import agent, models
...
class WorkerOptions:
    ...
    max_retry: int = 16
    """Maximum number of times to retry connecting to wizzpert."""
    ws_url: str = "ws://localhost:7880"
    """URL to connect to the wizzpert server.

    By default it uses ``wizzpert_URL`` from environment"""
    api_key: str | None = None
    """API key to authenticate with wizzpert.

    By default it uses ``wizzpert_API_KEY`` from environment"""
    api_secret: str | None = None
    """API secret to authenticate with wizzpert.

    By default it uses ``wizzpert_API_SECRET`` from environment"""
    ...
    """HTTP proxy used to connect to the wizzpert server.
...
class Worker(utils.EventEmitter[EventTypes]):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        opts.ws_url = opts.ws_url or os.environ.get("wizzpert_URL") or ""
        opts.api_key = opts.api_key or os.environ.get("wizzpert_API_KEY") or ""
        opts.api_secret = opts.api_secret or os.environ.get("wizzpert_API_SECRET") or ""

        if not opts.ws_url:
            raise ValueError("ws_url is required, or add wizzpert_URL in your environment")

        if not opts.api_key:
            raise ValueError("api_key is required, or add wizzpert_API_KEY in your environment")

        if not opts.api_secret:
            raise ValueError(
                "api_secret is required, or add wizzpert_API_SECRET in your environment"
            )
        ...
        self._api: api.wizzpertAPI | None = None
    ...
    async def simulate_job(
        ...
        from wizzpert.protocol.models import Room
        ...
        while not self._closed:
            try:
                ...
                if retry_count >= self._opts.max_retry:
                    raise RuntimeError(
                        f"failed to connect to wizzpert after {retry_count} attempts",
                    ) from None
                ...
                logger.warning(
                    f"failed to connect to wizzpert, retrying in {retry_delay}s", exc_info=e
                )

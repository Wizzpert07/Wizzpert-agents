...
from wizzpert.rtc.jupyter import display_room
...
def run_app(
    ...
    *,
    ...
    if IN_COLAB:
        ...
        if not jupyter_url:
            opts.ws_url = userdata.get("wizzpert_URL")
            opts.api_key = userdata.get("wizzpert_API_KEY")
            opts.api_secret = userdata.get("wizzpert_API_SECRET")
    else:
        opts.ws_url = os.environ.get("wizzpert_URL", "")
        opts.api_key = os.environ.get("wizzpert_API_KEY", "")
        opts.api_secret = os.environ.get("wizzpert_API_SECRET", "")

    if not jupyter_url and (not opts.ws_url or not opts.api_key or not opts.api_secret):
        raise ValueError(
            "Failed to get wizzpert_URL, wizzpert_API_KEY, or wizzpert_API_SECRET from environment variables. "  # noqa: E501
    ...
    if jupyter_url:
        ...
        async def fetch_join_tokens(url: str):
            async with aiohttp.ClientSession() as session:
                async with session.post(url) as response:
                    ...
                    return data["wizzpert_url"], data["user_token"], data["agent_token"]
        ...
        try:
            ...
            raise ValueError(
                ...
                "You can still use your own wizzpert_URL, wizzpert_API_KEY, and wizzpert_API_SECRET from environment variables instead."  # noqa: E501
            ) from None

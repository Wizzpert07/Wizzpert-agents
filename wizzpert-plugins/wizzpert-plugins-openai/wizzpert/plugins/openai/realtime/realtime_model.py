...
from wizzpert.agents import APIConnectionError, APIError, io, llm, utils
from wizzpert.agents.llm.tool_context import (
    get_raw_function_info,
    is_function_tool,
    is_raw_function_tool,
)
from wizzpert.agents.types import NOT_GIVEN, NotGivenOr
...
class RealtimeSession(
    ...
    async def _main_task(self) -> None:
        headers = {"User-Agent": "wizzpert Agents"}
            ...
            for previous_msg_id, msg_id in diff_ops.to_create:
                ...
                self.send_event(
                    ConversationItemCreateEvent(
                        ...
                        item=_wizzpert_item_to_openai_item(chat_item),
    ...
    def _handle_conversion_item_created(self, event: ConversationItemCreatedEvent) -> None:
        ...
        try:
            self._remote_chat_ctx.insert(
                event.previous_item_id, _openai_item_to_wizzpert_item(event.item)
            )
...
def _wizzpert_item_to_openai_item(item: llm.ChatItem) -> ConversationItem:
...
def _openai_item_to_wizzpert_item(item: ConversationItem) -> llm.ChatItem:

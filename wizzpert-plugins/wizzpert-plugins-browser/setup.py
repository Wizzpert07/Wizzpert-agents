...
about = {}
with open(os.path.join(here, "wizzpert", "plugins", "browser", "version.py")) as f:
    exec(f.read(), about)
...
class CMakeBuild(build_ext):
    def build_extension(self, ext: CMakeExtension) -> None:
        ...
        if sys.platform.startswith("darwin"):
            ...
            self.copy_tree(
                app,
                str(extdir / "wizzpert" / "plugins" / "browser" / "resources" / "lkcef_app.app"),
            )


setuptools.setup(
    name="wizzpert-plugins-browser",
    version=about["__version__"],
    description="Chromium Embedded Framework (CEF) for wizzpert Agents",
    long_description=(here / "README.md").read_text(encoding="utf-8"),
    long_description_content_type="text/markdown",
    url="https://github.com/wizzpert/agents",
    classifiers=[
    ...
    packages=setuptools.find_namespace_packages(include=["wizzpert.*"]),
    python_requires=">=3.9.0",
    install_requires=["wizzpert-agents>=1.0.0.dev5"],
    package_data={
        "wizzpert.plugins.browser": ["py.typed"],
        "wizzpert.plugins.browser.resources": ["**", "lkcef_app.app"],
    },
    project_urls={
